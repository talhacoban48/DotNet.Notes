...
select top 42 * from PromosyonUrun where KullanimDurum = 1 order by NEWID()

update PromosyonUrun set KullanimDurum = 1
...
SM.tanim,
SM.adres
from KatilimciPromosyon KP
inner join PotansiyelMusteri PM on KP.PotansiyelMusteriID = PM.ID
inner join PromosyonUrun PU on KP.PromosyonUrunID = PU.ID
inner join SistemMagaza SM on KP.MagazaID = SM.ID
